...
def rota_usuario_principal():
    return render_template('admin_usuario/admin_usuario.html')

#rota para pagina visualização 
@routes_web_usuario.route('/localizar', methods=["GET"])
def rota_usuario_pagina_localizar():
    form = UsuarioForm()
    return render_template("admin_usuario/admin_usuario_detalhe.html", form=form)

#rota para pagina de edição
@routes_web_usuario.route('/novo', methods=["GET"])
def rota_usuario_pagina_novo():
    form = UsuarioForm()
    return render_template("admin_usuario/admin_usuario_editar.html", form=form)
...
def rota_usuario_pagina_detalhe(id):
    ...
    sucesso, resultado, mensagem = usuario_lista_selecionado(id)
    if not resultado:
        resultado = Usuario(None).to_dict()

    return render_template("admin_usuario/admin_usuario_detalhe.html", usuario=resultado, mensagem=mensagem, aceitaFavoritos=aceitaFavoritos, favorito=favorito, rota=rota, idatalho=idatalho)
...
def rota_usuario_pagina_editar(id):
    ...
    if not resultado:
        resultado = Usuario(None).to_dict()
    return render_template("admin_usuario/admin_usuario_editar.html", usuario=resultado, mensagem=mensagem)
...
def rota_usuario_listar_todos():    
    sucesso, resultado, mensagem = usuario_listar_todos()
...
#rota para listar todos os usuários
@routes_web_usuario.route('/filtro_paginado', methods=["POST"])
def rota_usuario_listar_filtro_paginado():    
    ...
    sucesso, resultado, mensagem, totalRegistros, totalPagina = usuario_lista_filtrado_paginado(filtro, ordem, pagina, quantidade)

    print(f"totalRegistros: {totalRegistros}")
    print(f"totalPagina: {totalPagina}")
    print(f"pagina: {pagina}")
    print(f"quantidade: {quantidade}")

    return jsonify({"sucesso":sucesso,"dados":resultado, "pagina":pagina, "quantidade":quantidade, "ordem":ordem, "filtro":filtro, "totalRegistros":totalRegistros, "totalPagina":totalPagina, "mensagem":mensagem})
...
def rota_usuario_salvar_novo():
    ...
    sucesso, resultado, mensagem = usuario_salvar_novo(usuario)
...
def rota_usuario_alterar_existente():
    ...
    sucesso, resultado, mensagem = usuario_alterar_existente(usuario)
    return jsonify({"success":sucesso, "dados":resultado, "mensagem":mensagem})
...
def rota_contato_excluir_existente(id):
    sucesso, resultado, mensagem = usuario_excluir_existente(id)
    return jsonify({"success":sucesso, "dados":resultado, "mensagem":mensagem})
...
def rota_usuario_logon():
    ...
    sucesso, usuario, mensagem = usuario_logon(email)
    if usuario:
        if check_password_hash(usuario.senha, senha):
            ...
            session["usuario_tipo"] = usuario.tipo       # Tipo de usuário para filtros em paginas e menu
