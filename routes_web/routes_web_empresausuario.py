...
def rota_empresausuario_pagina_detalhe(id):
    ...
    sucesso, resultado, mensagem = empresausuario_lista_selecionado(id)
    if not resultado:
...
def rota_empresausuario_listar_todos():
    sucesso, resultado, mensagem = empresausuario_listar_todos()
...
def rota_empresausuario_salvar_novo():
    ...
    sucesso, resultado, mensagem = empresausuario_salvar_novo(EmpresaUsuario(None, idempresa, idusuario, papel, dt_cadastro, status))
    return jsonify({"success":sucesso, "dados":resultado, "mensagem":mensagem})
...
def rota_empresausuario_alterar_existente():
    ...
    sucesso, resultado, mensagem = empresausuario_alterar_existente(EmpresaUsuario(idempresa_usuario, idempresa, idusuario, papel, dt_cadastro, status))
    return jsonify({"success":sucesso, "dados":resultado, "mensagem":mensagem})
...
def rota_empresausuario_excluir_existente(id):
    sucesso, resultado, mensagem = empresausuario_excluir_existente(id)
    return jsonify({"success":sucesso, "dados":resultado, "mensagem":mensagem})
