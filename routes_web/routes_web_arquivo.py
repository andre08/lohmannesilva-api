...
def rota_arquivo_pagina_detalhe(id):
    ...
    sucesso, resultado, mensagem = arquivo_lista_selecionado(id)
    if not resultado:
...
def rota_arquivo_listar_todos():
    sucesso, resultado, mensagem = arquivo_listar_todos()
...
def rota_arquivo_salvar_novo():
    ...
    sucesso, resultado, mensagem = arquivo_salvar_novo(Arquivo(None, idestudo, nome, decricao, identificacao, localizacao_container, dt_importacao))
    return jsonify({"success":sucesso, "dados":resultado, "mensagem":mensagem})
...
def rota_arquivo_alterar_existente():
    ...
    sucesso, resultado, mensagem = arquivo_alterar_existente(Arquivo(idarquivo, idestudo, nome, decricao, identificacao, localizacao_container, dt_importacao))
    return jsonify({"success":sucesso, "dados":resultado, "mensagem":mensagem})
...
def rota_arquivo_excluir_existente(id):
    sucesso, resultado, mensagem = arquivo_excluir_existente(id)
    return jsonify({"success":sucesso, "dados":resultado, "mensagem":mensagem})
